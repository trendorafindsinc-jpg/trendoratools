...
import { ArrowRight, PieChart, Wallet, Target, BarChart3, Shield, CalendarCheck2 } from 'lucide-react';
    ...
    <div className="space-y-20 pb-8 animate-fade-in scroll-pad-nav lg:pb-0">
      <section className="grid lg:grid-cols-2 gap-10 lg:gap-12 items-center min-h-[70vh]">
        <div className="space-y-7">
          <div className="inline-flex items-center gap-2 px-4 py-2 glass-card">
            ...
            <span className="text-xs text-[var(--text-secondary)] font-medium">A Trendora product · LUCIA</span>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              to="/planner"
              className="px-6 py-3 rounded-xl font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] border border-[var(--glass-border)] hover:bg-[var(--nav-hover)] transition-all flex items-center gap-2"
            >
              <CalendarCheck2 size={18} /> Open Planner
            </Link>
          </div>
          {totalSaved > 0 && (
      ...
      <section className="space-y-10">
        <div className="text-center max-w-2xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold text-[var(--text-primary)] mb-3">Powerful tools. Simply done.</h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <Link key={to} to={to} className="glass-interactive p-5 space-y-3 group">
              <div className="w-10 h-10 rounded-xl bg-violet-500/15 border border-violet-500/20 flex items-center justify-center text-violet-400 group-hover:bg-violet-500/25 transition">
                <Icon size={20} />
              </div>
      ...
      <section className="glass-primary p-6 sm:p-10 relative overflow-hidden">
        ...
        <div className="relative grid md:grid-cols-2 gap-8 items-center">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 text-xs text-violet-400 font-medium">
              <CalendarCheck2 size={14} /> Planner + Insights
            </div>
            <h2 className="text-2xl font-bold text-[var(--text-primary)]">See the picture. Act on what matters.</h2>
            <p className="text-[var(--text-muted)] text-sm leading-relaxed">
              Insights summarizes cash flow, savings, bills, and debt from your records.
              Planner turns those numbers into the next actions — due bills, budget pressure, debt order, and goals to fund.
            </p>
            <div className="flex flex-wrap gap-2">
              <Link to="/insights" className="glass-button inline-flex">
                Open Insights <ArrowRight size={16} />
              </Link>
              <Link to="/planner" className="btn-secondary inline-flex">
                Open Planner
              </Link>
            </div>
          </div>
          <div className="glass-card p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold text-[var(--text-primary)]">Private by design</div>
                <div className="text-xs text-[var(--text-faint)]">Your financial records stay on your device by default.</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="glass-card p-3 text-[var(--text-muted)]">Exact calculations</div>
              <div className="glass-card p-3 text-[var(--text-muted)]">Local records</div>
              <div className="glass-card p-3 text-[var(--text-muted)]">Optional Lucia Cloud</div>
              <div className="glass-card p-3 text-[var(--text-muted)]">Actionable tools</div>
            </div>
          </div>
        </div>
      </section>
