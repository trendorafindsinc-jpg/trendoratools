...
const allowedParameters: Record<string, readonly string[]> = {
  navigation_click: ['destination', 'source_page'],
  tool_used: ['tool_name', 'action'],
  planner_used: ['action'],
  insights_viewed: ['period'],
  pwa_install: ['platform'],
  app_launch: ['launch_type'],
  lucia_id_sign_in: ['result'],
  lucia_id_sign_out: ['result'],
  lucia_id_create_account: ['result'],
  cloud_backup: ['result'],
  cloud_restore: ['result'],
};
...
export const analytics = {
  ...
  async event(name: string, params: AnalyticsParams = {}) {
    ...
    try {
      await ensureScript();
      window.gtag?.('event', name, cleanParams(name, params));
    } catch {
